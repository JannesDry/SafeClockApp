...
const DatabaseLocked = () => (
  <div className="flex flex-col items-center justify-center min-h-[50vh] p-8 text-center bg-white rounded-xl shadow-lg border-l-4 border-amber-500 max-w-2xl mx-auto mt-10">
    ...
    <p className="text-slate-600 mb-6 max-w-md">
      Your app is connected, but Firebase is blocking access. You need to enable
      "Public Mode" in your database rules.
    </p>
...
export default function App() {
  ...
  const [kioskLocation, setKioskLocation] = useState(() => {
    try {
      return localStorage.getItem('safeclock_device_name') || 'Main Office';
    } catch (e) {
      return 'Main Office';
    }
  });
  ...
  useEffect(() => {
    try {
      localStorage.setItem('safeclock_device_name', kioskLocation);
    } catch (e) {}
  }, [kioskLocation]);
  ...
  const verifyAdminPin = async () => {
    ...
    try {
      ...
      if (adminPinInput === '0000') {
        ...
        setAdminError('Error or Wrong PIN');
      }
    }
  };
        ...
        <div className="absolute inset-0 z-[100] bg-slate-900/80 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-2xl p-6 w-full max-w-sm">
            ...
            <input
              ...
              placeholder="PIN"
              autoFocus
              className="w-full text-center text-3xl font-bold border-2 rounded-xl py-3 mb-4"
              value={adminPinInput}
            ...
            <div className="grid grid-cols-2 gap-3">
              <button
                ...
                className="py-2 rounded-xl bg-slate-100"
              >
                Cancel
              </button>
              <button
          ...
          <div className="flex bg-slate-800 rounded-lg p-1">
            <button
              ...
              className={`px-4 py-1.5 rounded-md text-sm font-medium flex items-center gap-2 ${
                viewMode === 'manager' ? 'bg-indigo-600' : 'text-slate-400'
              }`}
            >
              Manager
            </button>
            <button
              ...
              className={`px-4 py-1.5 rounded-md text-sm font-medium flex items-center gap-2 ${
                viewMode === 'kiosk' ? 'bg-indigo-600' : 'text-slate-400'
              }`}
            >
              Kiosk
            </button>
...
function ManagerDashboard({ userId, kioskLocation, setKioskLocation }) {
  ...
  useEffect(() => {
    ...
    const unsubSettings = onSnapshot(
      ...
      (doc) => {
        if (doc.exists()) {
          ...
          if (doc.data().defaultShift) setNewEmpShift(doc.data().defaultShift);
        }
      },
      handleError
    );
    ...
    const unsubEmps = onSnapshot(
      query(collection(db, 'artifacts', appId, 'public', 'data', 'employees')),
  ...
  const generateCodes = async (targetDateStr) => {
    try {
      ...
      employees.forEach((emp) => {
        ...
        batch.update(
          doc(db, 'artifacts', appId, 'public', 'data', 'employees', emp.id),
          { currentCode: newCode, codeValidDate: targetDateStr }
        );
      });
      await batch.commit();
        ...
        {['status', 'overview', 'reports', 'settings'].map((tab) => (
          <button
            ...
            className={`px-4 py-2 rounded-md text-sm font-medium capitalize ${
              activeTab === tab ? 'bg-white text-indigo-600' : 'text-slate-600'
            }`}
          >
            ...
            <div className="bg-white p-6 rounded-xl border border-slate-200">
              ...
              <div className="flex gap-4">
                <button
                  ...
                  className="bg-amber-50 text-amber-800 px-4 py-2 rounded border border-amber-200 w-full"
                >
                  For Today
                </button>
                <button
                    ...
                    <a
                      href={`https://wa.me/${
                        emp.phoneNumber
                          ...
                          : ''
                      }?text=Hi ${encodeURIComponent(emp.name)}, code: ${
                        emp.currentCode
                      }`}
                      target="_blank"
                      rel="noreferrer"
          ...
          <div className="bg-white p-6 rounded-xl border border-slate-200">
            ...
            <div className="space-y-4 max-h-[500px] overflow-y-auto">
              {logs.slice(0, 20).map((log) => (
                <div
                  key={log.id}
                  className="flex items-start gap-3 text-sm pb-2 border-b"
                >
                  {log.photoUrl ? (
                    <img
                      src={log.photoUrl}
                    ...
                    <div className="flex justify-between">
                      <div className="font-medium">
                        ...
                        {log.action.includes('In') &&
                          isLate(
                            log.timestamp,
                            log.shiftStart || settings.defaultShift
                          ) && (
                      ...
                      {log.location && (
                        <div className="text-[10px] bg-slate-100 px-1 rounded text-slate-500">
                          {log.location}
                        </div>
                      )}
                    </div>
        ...
        <div className="max-w-md mx-auto bg-white p-6 rounded-xl border border-slate-200 space-y-4">
          ...
          <div>
            ...
            <p className="text-[10px] text-slate-400 mt-1">
              Set this differently on each tablet.
            </p>
          </div>
          <hr />
          <div>
          ...
          <div>
            <label className="text-xs">Daily Message</label>
            <input
              className="border w-full p-2 rounded"
...
function KioskMode({ userId, kioskLocation }) {
  ...
  useEffect(() => {
    ...
    if (db)
      onSnapshot(
        doc(db, 'artifacts', appId, 'public', 'data', 'settings', 'config'),
        (d) => d.exists() && setDailyMsg(d.data().dailyMessage),
        (err) => {
          if (err.code === 'permission-denied') setPermissionError(true);
        }
      );
  ...
  const handlePress = (n) => {
    if (inputCode.length < 6) setInputCode((prev) => prev + n);
  };
  const handleSubmit = async () => {
    ...
    try {
      ...
      await performAction(
        ...
        emp,
        action,
        emp.status === 'in' ? 'out' : 'in',
        photo
      );
      setStatus('success');
      setMessage(emp.status === 'in' ? 'Goodbye!' : dailyMsg || 'Welcome!');
      setTimeout(() => {
        setStatus('idle');
        setInputCode('');
      }, 3000);
  ...
  return (
    <div
      className={`min-h-full flex flex-col items-center justify-center p-4 transition-colors duration-500 ${
        status === 'locked' ? 'bg-black' : 'bg-slate-900'
    ...
    >
      ...
      <video
        ...
        className={`absolute inset-0 w-full h-full object-cover pointer-events-none transition-opacity duration-1000 ${
          status === 'locked' ? 'opacity-0' : 'opacity-20'
        }`}
      />
      ...
      <div
        className={`relative z-10 bg-white w-full max-w-md rounded-3xl shadow-2xl overflow-hidden ${
          status === 'locked' ? 'opacity-80' : 'opacity-100'
        }`}
      >
        <div
          className={`h-32 flex flex-col items-center justify-center p-6 ${
            status === 'success'
              ? 'bg-green-500'
              : status === 'error'
              ? 'bg-red-500'
        ...
        >
          ...
          {(status === 'locked' ||
            ...
            <>
              ...
              <div
                className={`text-4xl font-mono font-bold tracking-[0.5em] h-10 ${
                  status === 'locked' ? 'text-white' : 'text-slate-800'
                }`}
              >
                {inputCode.padEnd(6, '•')}
              </div>
        ...
        <div
          className={`p-6 grid grid-cols-3 gap-4 ${
            status === 'locked' ? 'bg-slate-900' : 'bg-white'
          }`}
        >
          {[1, 2, 3, 4, 5, 6, 7, 8, 9].map((n) => (
            <button
              ...
              className={`h-16 rounded-2xl text-2xl font-bold shadow-sm border ${
                status === 'locked'
                  ? 'bg-slate-800 border-slate-700 text-slate-200'
                  : 'bg-slate-50 border-slate-100 text-slate-700 active:bg-slate-200'
              }`}
            >
              {n}
            </button>
          ...
          <button
            ...
            className={`h-16 rounded-2xl font-medium text-sm ${
              status === 'locked'
          ...
          <button
            ...
            className={`h-16 rounded-2xl font-medium ${
              status === 'locked'
        ...
        >
          <button
            ...
            className={`w-full h-16 text-lg font-bold rounded-2xl shadow-lg ${
              status === 'locked'
                ? 'bg-indigo-900 text-indigo-200 disabled:bg-slate-800'
