...
import { 
  ...
  Camera, LogIn, LogOut, Settings, Bell, AlertTriangle, Clock4, Shield, MapPin, Database, ArrowLeft, ChevronRight, Zap, Send, Activity, Map, Calendar as CalendarIcon, X
} from 'lucide-react';
...
const runAutoPilot = async () => {
  ...
  try {
    const now = new Date();
    if (now.getHours() < 6 || (now.getHours() === 6 && now.getMinutes() < 30)) { console.log("Auto-Pilot: Too early..."); return false; }
    ...
    if (snap.exists()) {
      ...
      if (config.lastAutoGenerateDate !== todayStr) {
        // 1. GLOBAL HOLIDAY CHECK
        if (now.getDay() === 0 || isSAHoliday(now)) {
           console.log("Auto-Pilot: Holiday/Sunday. Skipping.");
        ...
        console.log("Auto-Pilot: Generating...");
        ...
        empSnap.docs.forEach(doc => {
          ...
          // 3. LEAVE CHECK (Specific Dates)
          if (emp.offDates && emp.offDates.includes(todayStr)) {
            console.log(`Skipping ${emp.name} (On Leave/Off)`);
            return;
          }
        ...
        await batch.commit();
        console.log(`Auto-Pilot: Generated for ${updates} employees.`);
        return true; 
      }
    }
...
export default function App() {
  ...
  useEffect(() => { if (!auth) return; const initAuth = async () => { try { await signInAnonymously(auth); } catch (e) { console.error(e); } }; initAuth(); onAuthStateChanged(auth, setUser); }, []);
...
function KioskMode({ userId, kioskLocation }) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (snap.empty) { setStatus('error'); setMessage('Invalid Code'); setTimeout(() => { setStatus('idle'); setInputCode(''); }, 2000); return; }
      ...
      setStatus('success'); setMessage(successMsg); setTimeout(() => { setStatus('idle'); setInputCode(''); }, 3000);
